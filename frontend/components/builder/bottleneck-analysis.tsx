...
export function BottleneckAnalysis({ build, onClose }: BottleneckAnalysisProps) {
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case "none": return "text-[#ffa828]";
      case "minor": return "text-yellow-400";
      case "moderate": return "text-orange-400";
      case "severe": return "text-[#ff4b4b]";
  ...
  const getSeverityBg = (severity: string) => {
    switch (severity) {
      case "none": return "bg-[#ffa828]/20 border-[#ffa828]/30";
      case "minor": return "bg-yellow-500/20 border-yellow-500/30";
      case "moderate": return "bg-orange-500/20 border-orange-500/30";
      case "severe": return "bg-[#ff4b4b]/20 border-[#ff4b4b]/30";
      default: return "bg-[#292524] border-[#44403c]";
    }
  };
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl max-h-[90vh] bg-[#1c1917] border border-[#292524] rounded-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-4 border-b border-[#292524] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#ff4b4b] rounded-xl flex items-center justify-center">
              <Zap className="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 className="text-lg font-semibold">Build Analysis</h2>
              <p className="text-sm text-stone-400">ML-powered performance prediction</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              <Loader2 className="w-8 h-8 text-[#ffa828] animate-spin mb-4" />
              <p className="text-stone-400">Analyzing your build...</p>
            </div>
          ) : error ? (
            <div className="text-center py-12">
              <AlertTriangle className="w-12 h-12 text-[#ff4b4b] mx-auto mb-4" />
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4">
                <span className="text-sm text-stone-400">Target Resolution:</span>
                <div className="flex gap-2">
                  {(["1080p", "1440p", "4k"] as const).map((res) => (
                    <button
                      key={res}
                      onClick={() => setResolution(res)}
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${resolution === res
              ...
              {/* FPS Prediction */}
              {prediction && (
                <div className="p-6 bg-[#1f1d1b] rounded-xl border border-[#292524]">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-semibold">Predicted Performance</h3>
                    <span className="text-3xl font-bold text-[#ffa828]">
                      {prediction.predicted_fps} FPS
                    </span>
                  </div>

                  {/* FPS Bar */}
                  <div className="relative h-4 bg-[#292524] rounded-full overflow-hidden mb-4">
                    <motion.div
                      initial={{ width: 0 }}
                    ...
                    />
                    <div className="absolute top-0 left-[41.6%] h-full w-px bg-stone-600" title="60 FPS" />
                    <div className="absolute top-0 left-[83.3%] h-full w-px bg-stone-600" title="120 FPS" />
                  </div>
                ...
                <div className={`p-4 rounded-xl border ${getSeverityBg(prediction.bottleneck_severity)}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-stone-300">{prediction.recommendation}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {ecosystem && (
                <div>
                  <h3 className="font-semibold mb-4 flex items-center gap-2">
                    <TrendingUp className="w-5 h-5 text-[#c678dd]" />
                    Ecosystem Comparison
                  </h3>
                  ...
                  <div className="space-y-3">
                    {ecosystem.comparisons.map((comp: ComparisonItem) => (
                      <div
                        key={comp.system}
                        className="flex items-center justify-between p-3 bg-[#1f1d1b] rounded-lg border border-[#292524]"
                      >
                        <div>
                        ...
                        <div className="text-right">
                          <div className={`text-sm font-medium ${comp.your_value_vs_system > 10
                            ? "text-[#ffa828]"
                          ...
                          <div className="text-xs text-stone-500">{comp.recommendation}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {ecosystem.best_value_alternative && (
                    <div className="mt-4 p-4 bg-[#292524] border border-[#44403c] rounded-lg">
                      <p className="text-sm text-[#ffa828]">
